...
class PaymentSettlementService:
    ...
    def summary(self):
        ...
        coverage = Decimal(str(matched_amount)) / Decimal(str(card_amount)) * 100 if card_amount else None
        return {"shopcaisse_card_payments": card_count, "shopcaisse_card_amount": str(card_amount), "sumup_transactions": self.db.execute("SELECT count(*) FROM sumup_transactions").fetchone()[0], "payouts": self.db.execute("SELECT count(*) FROM sumup_payouts").fetchone()[0], "qonto_credits":qonto_credits,"counts": counts, "matched_amount": str(matched_amount), "unmatched_amount": str(Decimal(str(card_amount))-Decimal(str(matched_amount))), "coverage_percent": str(coverage) if coverage is not None else None, "payout_balance": payout_states, "period":{"start":period[0],"end":period[1]},"revenue_included": False}
