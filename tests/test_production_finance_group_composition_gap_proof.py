...
def test_group_composition_gap_production_proof_is_pinned_locked_and_sanitized():
    root = Path(__file__).parents[1]
    workflow = (root / ".github/workflows/drcloud-os-finance-group-composition-gap-proof.yml").read_text()

    assert 'workflows: ["DrCloud OS Production"]' in workflow
    assert "group: drcloud-os-finance-group-composition-gap-proof" in workflow
    ...
    assert "from dr_cloud_sync.finance_group_composition_gap import group_composition_gap_funnel" in workflow
    ...
    assert "deduction_values_emitted':False" in workflow


def test_group_composition_gap_production_proof_clears_container_credentials_before_python():
    workflow = (
        Path(__file__).parents[1]
        / ".github/workflows/drcloud-os-finance-group-composition-gap-proof.yml"
    ).read_text()
