...
class FinanceProjection:
 ...
 def finance_cockpit(self,now=None):
  ...
  def revenue_build():
   ...
   rows=self._events_between(month,now) if sales_exist else []; sales_rows=[r for r in rows if r.get('event_kind')=='SALE']; count=len({(r['source'],r['external_sale_id']) for r in sales_rows}) if sales_exist else None; total=data['month_to_date']['value']; period=data['month_to_date']['period']
  ...
  def channels_build():
   ...
   for source,label in [('SHOPCAISSE','Magasin'),('PRESTASHOP','E-commerce')]:
    ...
    data.append({'channel':label,'source':source,'revenue':self._metric(self._sale_amount(selected) if known else None,'sale_events',revenue.get('month_to_date',{}).get('period'),'FRESH' if selected else 'ZERO_REEL' if known else 'UNAVAILABLE',sales_fresh),'sales_count':len({r['external_sale_id'] for r in selected if r.get('event_kind')=='SALE'}) if known else None})
  ...
  def payments_build():
   ...
   period={'start':month.isoformat(),'end':now.isoformat()}
   if self._has_table('sale_payments'):
    known=bool(self.db.execute("SELECT 1 FROM sale_payments WHERE source='SHOPCAISSE' LIMIT 1").fetchone())
    rows=self.db.execute("SELECT canonical_payment_type method,sum(CAST(amount AS NUMERIC)) amount,count(*) count FROM sale_payments WHERE source='SHOPCAISSE' AND quality_status='VALID' AND occurred_at>=? AND occurred_at<? GROUP BY canonical_payment_type ORDER BY amount DESC",(month.isoformat(),now.isoformat())).fetchall()
    result['by_method']=[{'method':r['method'],'count':r['count'],'amount':self._metric(Decimal(str(r['amount'])),'sale_payments',period,'FRESH',sales_fresh)} for r in rows]
    if known and not rows: result['by_method_coverage']='ZERO_REEL'
   if self.sumup_transactions and self._has_table('sumup_transactions'):
    rows=[r for r in self.sumup_transactions.rows() if r.get('timestamp') and month<=_iso(r['timestamp'])<now]; any_rows=bool(self.db.execute('SELECT 1 FROM sumup_transactions LIMIT 1').fetchone()); fresh=self._fresh_state(self._latest('sumup_transactions','imported_at'),now); amount=sum((_d(r['amount']) for r in rows if str(r.get('status')).upper() not in {'FAILED','CANCELLED','CANCELED'}),Decimal()) if any_rows else None; result['sumup_collected']=self._metric(amount,'sumup_transactions',period,coverage='FRESH' if any_rows else 'UNAVAILABLE',freshness=fresh);present.append((any_rows,fresh))
    if self._has_table('payment_settlements'):
     unsettled=[r for r in rows if str(r.get('status')).upper() not in {'FAILED','CANCELLED','CANCELED'} and not self.db.execute('SELECT 1 FROM payment_settlements WHERE sumup_transaction_id=? LIMIT 1',(r['sumup_transaction_id'],)).fetchone()]
     result['in_transit']=self._metric(sum((_d(r['amount']) for r in unsettled),Decimal()) if any_rows else None,'sumup_transactions/payment_settlements',period,'PARTIAL',fresh,method='transactions finales sans payout rapproché')
   if self.sumup_settlements and self._has_table('sumup_payouts'):
    rows=[r for r in self.sumup_settlements.rows() if r.get('payout_date') and month<=_iso(r['payout_date'])<now]; any_rows=bool(self.db.execute('SELECT 1 FROM sumup_payouts LIMIT 1').fetchone()); fresh=self._fresh_state(self._latest('sumup_payouts','imported_at'),now); paid=sum((_d(r['amount']) for r in rows if str(r.get('status')).upper() in {'PAID','SUCCESSFUL','COMPLETED'}),Decimal()) if any_rows else None; result['sumup_payouts']=self._metric(paid,'sumup_payouts',period,coverage='FRESH' if any_rows else 'UNAVAILABLE',freshness=fresh);present.append((any_rows,fresh))
   available=[x for x in present if x[0]]; fresh='STALE' if any(x[1]=='STALE' for x in available) else 'FRESH' if available else 'UNAVAILABLE';partial=bool(available) and len(available)<2
   return {**result,'status':status_for(bool(available),fresh,partial),'freshness':fresh,'coverage':'PARTIAL' if partial else fresh,'warning':None}
  ...
  def products_build():
   ...
   data=[{'product_key':k,'revenue':str(v['revenue']),'gross_margin':str(v['revenue']-v['cost']) if v['covered'] else None,'coverage':'FRESH' if v['covered'] else 'UNKNOWN'} for k,v in grouped.items()];partial=bool(data) and any(x['gross_margin'] is None for x in data);return {'status':status_for(bool(data),sales_fresh,partial),'freshness':sales_fresh,'coverage':'PARTIAL' if partial else 'FRESH' if data else 'UNAVAILABLE','warning':'Top marge limité aux produits avec cost_basis.' if partial else None,'top_by_revenue':sorted(data,key=lambda x:Decimal(x['revenue']),reverse=True)[:10],'top_by_margin':sorted([x for x in data if x['gross_margin'] is not None],key=lambda x:Decimal(x['gross_margin']),reverse=True)[:10]}
