...
def test_exact_match_proof_uses_dedicated_concurrency_and_deploy_lock():
    path = Path('.github/workflows/drcloud-os-finance-exact-match-funnel-proof.yml')
    text = path.read_text(encoding='utf-8')

    assert 'workflows: ["DrCloud OS Production"]' in text
    assert 'group: drcloud-os-finance-exact-match-funnel-proof' in text
    ...
    assert lock in text
    assert 'flock 9' in text
    assert text.index('source "$repo/deploy/ovh/deployment-environment.sh"') < text.index(lock)
    assert text.index(lock) < text.index('deployed_sha="$(git -C "$repo" rev-parse HEAD)"')
    assert text.index('flock 9') < text.index('docker compose exec -T')
