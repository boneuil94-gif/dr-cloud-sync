...
class PurchaseCostLedger:
    ...
    def map_product(self,supplier_id,reference,product_key,*,ean=None,actor="authenticated",source="MANUAL"):
        ...
        with self.db:self.db.execute("""INSERT INTO supplier_product_mappings VALUES(?,?,?,?,?,'MATCHED',?,?,?,?) ON CONFLICT(supplier_id,supplier_reference) DO UPDATE SET supplier_ean=excluded.supplier_ean,product_key=excluded.product_key,status='MATCHED',source=excluded.source,actor=excluded.actor,updated_at=excluded.updated_at""",(mid,supplier_id,reference,ean,product_key,source,actor,stamp,stamp))
    ...
    def create_invoice(self,data,lines,actor="authenticated"):
        ...
        try:
            with self.db:
                self.db.execute("INSERT INTO supplier_invoices VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)",values)
                for item in lines:
                    product,status=self.resolve(supplier,item.get("supplier_reference"),item.get("ean"),item.get("product_key"))
                    self.db.execute("INSERT INTO supplier_invoice_lines VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)",("sil:"+str(uuid.uuid4()),iid,product,item.get("supplier_reference"),str(item.get("description") or ""),*(money(item.get(x)) if item.get(x) not in (None,"") else None for x in ("quantity","unit_price_ht","tax_rate","tax_amount","total_ht","total_ttc")),status,item.get("receipt_line_id")))
    ...
    def allocate_sale(self,sale_event_id,product_key,quantity):
        ...
        with self.db:
            ...
            for lot in lots:
                ...
                self.db.execute("INSERT INTO sale_cost_allocations VALUES(:allocation_id,:sale_event_id,:product_key,:cost_lot_id,:quantity,:unit_cost,:total_cost,:allocation_method,:created_at)",row)
            ...
            if remaining:self.db.execute("INSERT OR REPLACE INTO purchase_cost_diagnostics VALUES(?,?,?,?,?)",("diag:"+str(uuid.uuid4()),"SALE_COST_UNCOVERED",sale_event_id,json.dumps({"quantity":str(remaining)}),now()))
